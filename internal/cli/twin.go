...
// gets its own directory under ~/.water/twins/<id>, so its fictional
// connector records, decision cards and classification cache never land in
// the real twin's database (a demo "github" PR would otherwise sit next to
// the real GitHub connector's rows under the same source name).
